...
def tf_cov(data):
    ...
    k = data.get_shape()[2].value

    mean_data = tf.reduce_mean(data, axis=2, keep_dims=True)  # (batch_size, num_point, 1, 3)
    mx = tf.tile(mean_data, [1, 1, k, 1])
    data_center = tf.subtract(data, mx)
    data_cov = tf.matmul(tf.transpose(data_center, perm=[0, 1, 3, 2]), data_center) / tf.cast(tf.shape(data)[2]-1, tf.float32)
    data_cov = tf.reshape(data_cov, shape=[batch_size, -1, 9])
